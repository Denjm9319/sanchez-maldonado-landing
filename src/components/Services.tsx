...
function IconVoice() {
  return (
    <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4">
      <path
        d="M5 12a7 7 0 0 0 14 0M12 16v3.5M9 19.5h6"
        strokeLinecap="round"
      />
...
function IconAutomation() {
  return (
    <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4">
      <path
        d="M8.5 8.5a4 3 0 0 1 7 0M15.5 15.5a4 3 0 0 1-7 0"
        strokeLinecap="round"
      />
      ...
      <div className="grid gap-[clamp(18px,2.4vw,26px)] [grid-template-columns:repeat(auto-fit,minmax(min(100%,240px),280px))]">
        {SERVICES.map((s, i) => (
          <Link
            key={s.n}
            to="/proyectos"
            className="group bg-white rounded-[16px] overflow-hidden border border-navy/[0.08] block"
          >
            <span
              className={`flex items-center justify-center aspect-[4/3] ${TINTS[i % TINTS.length]} text-navy transition-transform duration-500 ease-[cubic-bezier(.22,.61,.36,1)]`}
            >
            ...
            <span className="block pt-[18px] px-5 pb-[22px]">
              ...
              <span className="text-[13px] text-teal mt-3 block group-hover:text-navy">Ver ejemplos →</span>
            </span>
          </Link>
        ))}
      </div>
